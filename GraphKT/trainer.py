...
from model import GraphKTModel
    ...
    with tf.Session(config=run_config) as sess:
        ...
        if train_dkt:
            ...
            if not os.path.exists(tag_path):
                raise Exception("Cannot find tag file: " + tag_path)
            ...
            if ckpt is None:
                raise Exception("CKPT need, cannot find checkpoint in: " + checkpoint_dir)
            ...
            while not test_data_generator.end:
                ...
                binary_pred, pred = model.evaluate(
                    sess,
                    features_answer_index,
                    target_answers,
                    seq_lens,
                    hist_neighbor_index,
                    test_step
            ...
            print("\ntest auc={0}, accuracy={1}, precision={2}, recall={3}".format(
                auc_value,
                accuracy,
                precision,
                recall
            ...
            print(model_dir)
            write_log(args, model_dir, auc_value, accuracy, 0, name='test_')

def save(global_step,sess,checkpoint_dir,saver):
    model_name = 'GraphKTModel'
