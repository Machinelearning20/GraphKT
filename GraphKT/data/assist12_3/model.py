...
from graph_layers import Sum, Concat
import math

class GraphKTModel(object):
    def __init__(self, args):
        ...
        # Accept either a CLI string like "[0.8,0.8,1]" or a Python list from defaults/config.
        ...
        self.feature_embedding = tf.get_variable("feature_embedding", [self.feature_answer_size, self.embedding_size],initializer=tf.glorot_uniform_initializer())
        ...
        if args.aggregator == 'sum':
            self.aggregator_class = Sum
        elif args.aggregator == 'concat':
            self.aggregator_class = Concat
        else:
    ...
    def build_model(self):
        ...
        with tf.variable_scope('ni'):
            w1 = tf.get_variable('atn_weights_1',[hidden_size, 1], initializer=tf.glorot_uniform_initializer())
            w2 = tf.get_variable('atn_weights_2',[hidden_size, 1],initializer=tf.glorot_uniform_initializer())
    ...
    def hist_neighbor_sampler(self,input_embedding):
        ...
        hist_neighbors_features =tf.reshape(tf.gather(O, temp_hist_index, batch_dims=1),[-1,self.max_step,self.hist_neighbor_num,input_embedding.shape[-1]])
    ...
    def hist_neighbor_sampler1(self, input_q_emb, next_q_emb, qa_emb):#sample based on question similarity
        ...
        hist_neighbors_features =tf.reshape(tf.gather(qa_emb, temp_hist_index, batch_dims=1), [-1, self.max_step, self.hist_neighbor_num, qa_emb.shape[-1]])
        return hist_neighbors_features
