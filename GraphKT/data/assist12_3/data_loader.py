...
def build_adj_list(train_seqs,test_seqs,skill_matrix,qs_num):
    ...
    for seqs in [train_seqs,test_seqs]:
        for seq in seqs:
            ...
            save_question =[]
            for step in seq:
                if step[1] not in save_question:
                    save_question.append(step[1])
        ...
        if index < s_num:   #skill
            ...
            if len(neighbors)>0:
                if len(neighbors) >= q_neighbor_size:
                    ...
                    if len(save_skill)>=q_neighbor_size:
                        ...
                        for i in range(len(temp)):
                            question_neighbors[index][i+changdu+1]=temp[i]
                else:
